...
namespace KitaKo.Services
{
    public class KnapsackService
    {
        ...
        public ExpenseOptimizationResult OptimizeExpenses(List<Expenses> expenses, decimal budget)
        {
            ...
            // ── FIX: work in centavos to preserve decimal precision ───────────
            // Cap at a safe DP table size to avoid OOM on very large budgets.
            const int MaxCapacityCentavos = 10_000_000; // ₱100,000 max granularity
            int W = (int)Math.Min(budget * 100m, MaxCapacityCentavos);

            // Precompute weights & values
            ...
            // ── 0/1 Knapsack DP ──────────────────────────────────────────────
            // Use 1-D rolling array to save memory: O(W) space instead of O(n·W)
            int[] dp = new int[W + 1];
            ...
            for (int i = 0; i < n; i++)
            {
                int wi = weights[i];
                int vi = values[i];
                // Traverse right-to-left to ensure each item is used at most once
                for (int w = W; w >= wi; w--)
                {
                    if (dp[w - wi] + vi > dp[w])
                        dp[w] = dp[w - wi] + vi;
            ...
            // ── Backtrack: rebuild full table only for backtracking ───────────
            // Re-run with 2-D table (only needed for the traceback)
